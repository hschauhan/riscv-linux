...
void __noreturn do_sse(unsigned long evt, struct sse_interrupted_state *i_state,
		       sse_event_handler *handler, void *arg)
{
	int ret;
	enum sbi_sse_event_handler_sts handler_ret;
...
sse_event_complete:
	...
	handler_ret = ret ? SBI_SSE_HANDLER_FAILED : SBI_SSE_HANDLER_SUCCESS;
	sbi_ecall(SBI_EXT_SSE, SBI_SSE_EVENT_COMPLETE, evt, handler_ret, 0, 0,
	          0, 0);
 ...
 */
static unsigned long *alloc_vmap_stack(size_t stack_size, int node)
{
	void *p;
...
{
	return alloc_vmap_stack(size, cpu_to_node(cpu));
}
